...
- Select a single rendering/windowing backend stack per platform family:
  - macOS / Windows / Linux desktop: GLFW + OpenGL 3 (or Metal on macOS as an optional path).
  - iOS: Apple Metal + UIKit view controller host (no GLFW; iOS has its own app lifecycle).
...
- `render-backend`: The selection and abstraction of rendering/windowing backends per platform (GLFW+OpenGL3 for desktop, Metal+UIKit for iOS), including ImGui backend wiring.
...
- New build-time dependencies: CMake ≥ 3.24, a C++17 compiler per platform, GLFW (desktop), Xcode toolchain (iOS/macOS).
