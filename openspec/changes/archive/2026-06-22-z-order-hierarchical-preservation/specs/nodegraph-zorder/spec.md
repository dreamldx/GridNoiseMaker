...
The system SHALL automatically adjust z-order values when nodes are selected with the mouse, bringing selected nodes to the foreground while preserving relative z-order hierarchy among other nodes.
...
- **AND** selection SHALL trigger hierarchical z-order reordering as described above
