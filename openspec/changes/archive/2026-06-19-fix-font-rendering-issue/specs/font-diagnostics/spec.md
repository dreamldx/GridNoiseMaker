...
\end- **WHEN** the user presses Ctrl+Shift+D (desktop) or three-finger tap (iOS)
...
\end- **WHEN** the font diagnostic overlay is visible
...
\end- **WHEN** the diagnostic overlay is in detailed mode (activated by clicking)
...
-c- **AND** SHALL not trigger font atlas rebuild or other side effects
