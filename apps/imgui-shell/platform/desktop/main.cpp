...
// Captured during runApp setup so the font-atlas rebuild callback can find
// the Vulkan device handle. See specs/font-rendering "Platform supplies the
// rebuild callback during init".
VkDevice g_imguiDevice = VK_NULL_HANDLE;

void rebuildFontAtlasDesktop() {
    // Drain previous-frame GPU work so the old font texture has no in-flight
    // readers, then destroy + re-add the atlas + recreate the GPU texture.
